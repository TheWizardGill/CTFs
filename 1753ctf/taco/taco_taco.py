...
def encrypt(key, nonce, pt):
    ...
    for i in range(blockcount(pt)):
        out += xor_bytes(salsa20_block(key, nonce,
                         struct.pack('>Q', i)), nthblock(pt, i))
    return out
...
def main():
    ...
    nony = token_bytes(8)
    print(nony)
    print(encrypt(KEY, nony, (pt + FLAG).encode()).hex())
